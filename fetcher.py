...
# TSE regular session: 09:00–13:30 (continuous trading ends 13:25,
# closing call auction executes at 13:30).
# Bars outside this window and zero-volume placeholder bars generated by
# Shioaji during the closing auction collection period (13:26–13:29)
# are stripped before resampling.
...
class ShioajiDataFetcher:
    ...
    def _normalise(kbars) -> pd.DataFrame:
        ...
        # ── Step 4: strip non-session and zero-volume bars ────────────────
        # Shioaji injects placeholder bars during the closing call auction
        # collection period (13:26–13:29 CST) with Volume=0 and OHLC set to
        # the last traded price. Including them in resampling corrupts the
        # Close of the last 5-min bar and generates phantom candles on the
        # chart. Filter to the regular session window and remove V=0 rows.
        bar_times = df.index.time
        df = df[
            (bar_times >= _SESSION_START) &
            (bar_times <= _SESSION_END) &
            (df["Volume"] > 0)
        ]
